...
WARDS = {
    ...
    "shinjuku": {
        ...
        "ward_faq": ("不動産管理会社ですが、同業の支援実績はありますか？", "あります。東京都内の不動産管理会社で、書類検索・転記工数を月40時間削減し、月額基幹システムの解約まで実現した事例があります（契約金額150万円・約2ヶ月）。"),
    ...
    "chiyoda": {
        ...
        "ward_faq": ("紙の書類が大量にあるのですが、データ化から頼めますか？", "はい。紙契約書を全件スキャンし、相手先・開始日・種別を一覧化して横断検索できる状態まで構築した実績があります（埼玉県・不動産会社／契約金額350万円・約3ヶ月）。"),
    ...
    "ota": {
        ...
        "ward_faq": ("製造業の支援実績はありますか？", "製造業の見積業務について、月1,500時間規模の業務で作業時間50〜60%削減を目標とする支援を設計したご提案実績があります（実施前のご提案例・想定契約金額300万円）。既存のGoogle Workspace上に構築する、内製化しやすい構成です。"),
...
STEPS = [
    ("STEP 0", "無料簡易診断", "オンライン・約2時間", "0円", "業務の概要を伺い、AI化の期待効果の目安をその場でご提示します。"),
    ("STEP 1", "実地調査（AI化アセスメント）", "調査工数30〜90時間・約1〜3ヶ月", "50〜150万円", "現場で業務を棚卸しし、AI化ロードマップと削減効果の試算を納品。調査のみのご利用も可能です。"),
    ("STEP 2", "駐在型AI化支援", "準委任・月60時間・3〜6ヶ月", "参考 100万円/月", "ロードマップに沿って、コンサルタントが現場で手を動かしながらAI化を実行します。"),
    ("STEP 3", "内製化（AI研修）", "4回×2.5時間", "30万円/名", "構築済みの自社業務フローを教材に、社員の皆さまが使い方と保守運用を習得。助成金で実質1/4以下になる場合も。"),
]

CASES = [
    ("不動産管理 A社（東京都）", "従業員10〜50名", "書類検索・契約書整形が手作業で、月額基幹システムのコストも負担に。",
     "書類の自動リネーム・名寄せ・1物件1ページ帳票の自動生成を構築し、現場に引き継ぎ。",
     "工数 月40時間削減 ＋ 基幹SaaS解約", "契約金額150万円・現地訪問5回×約2ヶ月"),
    ("不動産 B社（埼玉県）", "従業員10〜50名・M&Aに伴う情報統合", "契約書が紙・PDFでバラバラ。書類探しが属人化し、大量の紙データのSaaS移行が必要に。",
     ...
     "書類の属人化解消 ＋ PMIデータ移行完了", "契約金額350万円・現地訪問7回×約3ヶ月"),
]

COMMON_FAQS = [
    ("費用はどのくらいかかりますか？", "無料簡易診断（0円）で効果の目安を確認いただいた上で、実地調査は50〜150万円、駐在型支援は月60時間稼働で参考100万円/月です。納品後のサブスクリプション費用はありません。"),
...
SUBSIDY_NOTE = "STEP 3のAI研修は、人材開発支援助成金「事業展開等リスキリング支援コース」の対象になる場合があります（中小企業：経費助成75%＋賃金助成1,000円/時）。例えば5名受講・受講料150万円の場合、実質負担は約32.5万円。※受給可否・金額は申請結果により変動します。訓練開始1ヶ月前までの申請が必要なため、お早めにご相談ください。"
...
<section>
  ...
  <h2>駐在型——現場に入り、実装までやり切るAIコンサルティング</h2>
...
<section>
  ...
  <h2>進め方と料金——各ステップで判断できます</h2>
  <div class="grid2">{steps}</div>
  <div class="note"><p><b>助成金のご案内:</b> {esc(SUBSIDY_NOTE)}</p></div>
</section>
...
<section class="cta-box">
  ...
  <h2>まずは、無料の簡易診断から</h2>
  <p>オンライン・約2時間で、貴社のAI化の期待効果の目安をご提示します。診断だけのご利用も歓迎です。{esc(ward_name)}の企業さまからのご相談をお待ちしています。</p>
  <a class="btn-main" href="{mailto('無料簡易診断の申込（' + ward_name + '）')}">無料簡易診断を申し込む →</a>
...
<div class="hero">
  ...
  <a class="btn-main" href="{mailto('無料簡易診断の申込（' + w['name'] + '）')}">無料簡易診断を申し込む →</a>
  <p class="hero-meta">オンライン約2時間・0円／{esc(w['access'])}</p>
</div>
